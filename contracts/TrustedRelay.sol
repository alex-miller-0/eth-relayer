...
contract TrustedRelay {
  ...
  event ChangeFee(uint newAmount, uint timestamp);
  ...
  function depositERC20(bytes32 m, uint8 v, bytes32 r, bytes32 s, address token, uint amount, uint[2] chainIds) public payable {
  ...
  function relayedDepositERC20(bytes32 m, uint8 v, bytes32 r, bytes32 s, address[2] addrs, uint amount, uint[2] chainIds) public {
    assert(owners[msg.sender] == true);
  ...
  // addrs = [ token, originalSender ]
  function undoDepositERC20(bytes32 m, uint8 v, bytes32 r, bytes32 s, address[2] addrs, uint amount, uint[2] chainIds) public {
    assert(owners[msg.sender] == true);
    assert(m == keccak256(chainIds[0], chainIds[1], addrs[0], amount, addrs[1]));
    assert(chainIds[0] == chainId);
    address sender = ecrecover(m, v, r, s);
