...
function saveConfig(originId, destId, originUrl, destUrl) {
  ...
  jsonfile.writeFile('./networks.json', networks, { spaces: 2 }, (err) => {
    if (err) {
      ...
      console.log('Success! Networks saved to ./networks.json\n');
    }
  });
}
...
function parseArgv() {
  ...
  let destination;
  try {
    const originPort = parseInt(process.argv[2], 10);
    origin = `http://localhost:${originPort}`;
  } catch (err) {
    origin = process.argv[2];
  }
  try {
    const destPort = parseInt(process.argv[3], 10);
...
const networkIds = { origin: null, destination: null };
if (process.argv.length !== 4) {
  ...
  getNet(origin, 1, (err, res) => {
    if (err) {
      console.log('Error getting id of origin network:', err);
    } else {
      const parsed = JSON.parse(res);
      networkIds.origin = parsed.result;
      getNet(destination, 2, (err2, res2) => {
        if (err2) {
          console.log('Error getting id of destination network:', err2);
        } else {
